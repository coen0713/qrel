...
class ContaminationConfig(StrictModel):
    """The M3 headline experiment, as one config file.

    PLAN.md M3's acceptance criterion is that the results table is "generated by
    one command from a config file" — so every choice the experiment makes lives
    here, including the ones the pre-registration fixed.
    """

    name: str = "contamination-scifact"
    dataset: str = "scifact"
    split: str | None = None
    seed: int = 0

    #: Paths to synthetic query sets produced by `reval contaminate generate`.
    synthetic: list[str] = Field(default_factory=list)

    #: Chunkers to sweep. Four of them tests H4 (does the eval set change the
    #: ranking of chunking strategies?); one is enough for H1-H3.
    chunkers: list[Literal["fixed", "recursive", "semantic", "parent"]] = Field(
        default_factory=lambda: ["fixed"]
    )
    retrievers: list[Literal["bm25", "dense"]] = Field(default_factory=lambda: ["bm25", "dense"])

    encoder: EncoderConfigModel = Field(default_factory=EncoderConfigModel)
    eval: EvalConfig = Field(default_factory=EvalConfig)
    bootstrap: BootstrapConfig = Field(default_factory=BootstrapConfig)

    #: Overlap strata. 10 is the pre-registered value.
    n_overlap_bins: int = 10

    #: The measure H1 is stated on. Others are still reported.
    primary_measure: str = "recall@10"

    @classmethod
    def from_yaml(cls, path: str | Path) -> ContaminationConfig:
        data = yaml.safe_load(Path(path).read_text(encoding="utf-8")) or {}
        return cls.model_validate(data)
